...
/**
  ...
  */
class ScalafixInterfacesClassloader(parent: ClassLoader) extends ClassLoader(null) { // scalafix:ok
  override def findClass(name: String): Class[_] =
    if (name.startsWith("scalafix.interfaces")) parent.loadClass(name)
    else throw new ClassNotFoundException(name) // scalafix:ok
}
